...
WORK_MIN = 1   # Set your desired countdown time in minutes
timer_id = None
reps = 0
session_history = []

def reset_timer():
    global timer_id, reps, session_history
    if timer_id:
...
def count_down(count):
    ...
    if count > 0:
        ...
        session_history.append(f"Session {reps} finished at {end_time}")
...
def show_statistics():
    ...
    Label(stats_window, text=f"Total Sessions Completed: {reps}", font=("Arial", 15)).pack(padx=20, pady=10)

    total_time = reps * WORK_MIN
    Label(stats_window, text=f"Total Time Studied: {total_time} minutes", font=("Arial", 15)).pack(pady=5)

    if session_history:
        Label(stats_window, text="Session History:", font=("Arial", 12, "bold")).pack(pady=5)
        for entry in session_history:
            Label(stats_window, text=entry, font=("Arial", 12)).pack(anchor="w", padx=20)
